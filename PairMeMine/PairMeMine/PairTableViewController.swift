...
class PairTableViewController: UITableViewController {
    ...
    var randomizedPersons: [(Person, Person?)] = []
    ...
    override func viewWillAppear(animated: Bool) {
        randomizedPersons = PersonController.sharedInstance.randomizePersons()
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCellWithIdentifier("pairedPeopleCell", forIndexPath: indexPath) as? PairedPeopleTableViewCell {
        
            let pair = self.randomizedPersons[indexPath.row]
            
            cell.fillLabels(pair.0, user2: pair.1)
